	...
	return {
		...
		initContentClicked: function() {
			this.contentClicked.push(function(evt, node) {
				if(!JusTo.ui.Node.canSelect(node)) return;
...
JusTo.ui.Node.prototype._getContent = function() {
	...
	for(i=0; i<element.childElementCount; i++) {	
		if(element.children[i].className.indexOf("content") >= 0) return element.children[i];
	}
};
...
JusTo.ui.NodeFilter.leaf = function(node) {
	return node.urlSubItems === undefined && (!node.hasChildren());
};

JusTo.ui.NodeFilter.all = function(node) {
	return true;
};